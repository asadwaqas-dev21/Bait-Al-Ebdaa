...
function Header({ menuOpen, setMenuOpen }) {
  ...
  return <>
    <header className={`site-header ${scrolled || menuOpen ? "site-header--solid" : ""}`}>
      ...
      <a className="outline-button header-cta" href="#contact">{dict.nav.startProject} <ArrowUpRight size={15} /></a>
    </header>
    <div id="site-menu" className={`menu-overlay ${menuOpen ? "is-open" : ""}`} aria-hidden={!menuOpen}>
      <div className="menu-overlay__main">
        <p className="micro">{dict.nav.navigate}</p>
        {["Home", "Studio", "Services", "Projects", "Process", "Contact"].map((item, i) => (
          <a key={item} href={item === "Home" ? "#top" : `#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)}><span>0{i + 1}</span>{dict.menuItems[i]}</a>
        ))}
      </div>
      <div className="menu-overlay__aside">
        <div><p className="micro">{dict.nav.studio}</p><p style={{whiteSpace: "pre-wrap"}}>{dict.nav.studioLocation}</p></div>
        <div><p className="micro">{dict.nav.newEnquiries}</p><a href="mailto:studio@arcandgrain.ae">studio@arcandgrain.ae</a><a href="tel:+97140000000">+971 4 000 0000</a></div>
        <p className="menu-note">{dict.nav.menuNote}</p>
      </div>
    </div>
  </>;
}
